# Spoonerism — Build Specification

> Portfolio app 116, batch pending. This document is the complete brief for
...
**One-line positioning:** Tap a word that swapped its first letter on a saved painting's maker or title.

| Field | Value |
| --- | --- |
| Product name | Spoonerism |
| Bundle identifier | `com.spoonerism.marrow` |
| Domain | https://spoonerism-marrow.pro |
| Contact URL | https://spoonerism-marrow.pro/contact-us |
| Deployment target | iOS 17.0 |
| Swift version | 6.2, strict concurrency `complete` |
| Devices | iPhone and iPad, portrait |
| Interface style | Light |
| Asset prefix | `spn_` |
| User-Agent | `Spoonerism/1.0 (iOS; +https://spoonerism-marrow.pro)` |
...
8. **The app must build clean** with
   `xcodegen generate && xcodebuild -scheme Spoonerism -destination 'generic/platform=iOS' build`.
...
A lecturer taps a word that traded its first letter so the saved painting files as mended.

### 2.1 User flow

1. Tap one of the two heads that traded first letters on the seeded line
2. Watch both letters right and the painting file on Saved as Mended
3. Open Explore as a sheet and save another work from the catalog
4. Spoon a new line from paintings that are not Mended
5. Open Saved to review MendMarks and MuffMarks
6. Open Settings to read the Prado credit

### 2.2 Essential behaviour

- Saved works from one catalog
- Spoonerism quiz on artist or title, never both fields at once
- MendMarks and MuffMarks stay reviewable
- Local only
- No shop
- One collection voice

---

## 3. Uniqueness assignment for Spoonerism

| Axis | Assigned value |
| --- | --- |
| Architecture | **Spoonerism ADT fold (Idle | Spooned | Mended); the onset is a fold over Works; Spoon writes a Line that is artist XOR title with the first letters of two Heads swapped and folds Idle to Spooned; Mend writes a MendMark when the tapped Head is one of the swapped pair, rights both letters, and folds Spooned to Mended; a miss writes a MuffMark and keeps the Line; Mend on Idle is refused; a second Spoon while Spooned is refused; Spoon samples a Work that is not Mended whose chosen field has at least two tokens with unlike first letters; empty onset writes Fluent** |
| UI approach | **SwiftUI pure · take marrow** |
| Naming convention | **Spoonerism / marrowsky lexicon** |
| File organization | **By onset role (Onset, Work, Line, Head, MendMark, MuffMark) · marrow** |
| Dependency strategy | **None** |
| Design direction | **Soft card daylight · take marrow** |
| Typography | **SF Pro** |
| Navigation pattern | **Onset-locked chrome (the spoonered line never leaves; Explore, Saved and Settings arrive as sheets; spoon and mend fuse on Quiz) · marrow** |
| AI art style | **3D glass render glassmorphism · take marrow** |
| Functional twist | **Spoon-then-mend (Spoon samples a Work that is not Mended whose chosen field has at least two tokens with unlike first letters; a Line is artist XOR title with the first letters of two Heads swapped; Mend writes a MendMark when the tapped Head is one of the swapped pair, rights both letters, and folds Spooned to Mended; a miss writes a MuffMark and keeps the Line; Mend on Idle is refused; Spoon on a field with fewer than two unlike first letters is refused)** |
...
**Core** — A lecturer taps a word that traded its first letter so the saved painting files as mended.

**Audience** — People who already saved museum paintings and want to mend swapped opening letters on this device, not right two transposed sorts, not face retrograde words, and not walk a museum site.
...
**Twist** — Spoon-then-mend. Quiz keeps the onset. Spoon draws one saved painting that is not Mended and whose picked field holds two or more words with unlike opening letters, then paints those words as a Line: maker or title, never both, with the first letters of two Heads exchanged. Every Head can be hit. Mend records a MendMark when the hit Head is one of the exchanged pair, rights both letters, and moves Spooned to Mended. Hitting any other Head records a MuffMark, greys that Head, and leaves the Line up. Mend does nothing before Spoon. A later Spoon while Spooned does nothing. Explore adds a painting as Idle; a duplicate object id only focuses that row. Mended paintings sit on Saved and leave Spoon. Undo peels the newest MendMark or MuffMark. Spoon with no usable painting prints Fluent. Launch already paints a live Line, and the first Head hit can file. The job on home is mend-the-onset, not right-the-sort and not face-the-furrow. Saved lists MendMarks together with MuffMarks. Settings names Museo Nacional del Prado. No grade. No shop.

**Why this is not a repeat** — This is not a reskin of Anastrophe, Hyperbaton, or Boustrophedon. Those folds move or reverse whole words. Home here leaves word order alone and swaps only the first letter of two Heads on artist XOR title. The first tap after seed writes a MendMark on a live Line, so the primary CTA is not gated. Explore, Saved, and Settings arrive as sheets on an onset-locked Quiz; there is no three-tab stamp. Soft card daylight, SwiftUI pure, and glass art reuse take artquiz11 because take artquiz7 is already held. Prado is a new collection voice. Onset, Head, MendMark, MuffMark, Fluent, and marrowsky do not appear on nearby folds.
...
The onset is one Codable algebraic fold over Works whose cases are Idle, Spooned, and Mended, and every user verb is a total function on that crate. Spoon samples one Work that is not Mended whose chosen field has at least two tokens with unlike first letters, writes a Line that is artist XOR title with the first letters of two Heads swapped, and folds Idle to Spooned. Mend writes a MendMark when the tapped Head is one of the swapped pair, rights both letters, and folds Spooned to Mended. A miss writes a MuffMark, greys that Head, and keeps the Line. Mend on Idle is refused, a second Spoon while Spooned is refused, and an onset with no usable painting writes Fluent. Explore adds a Work as Idle and a duplicate object id only focuses that row. Mended works rest on Saved and leave the Spoon pool. Undo peels the newest MendMark or MuffMark.
...
One hundred percent SwiftUI. No UIViewRepresentable, no AVFoundation preview, no custom drawing layer beyond stock Image and the Head buttons. Quiz is photography-first hero-rail: one large painting tile, caption under the tile, the spoonered Line as the mechanic, a recent MendMark rail, and one MuffMark count. Pill CTA. Soft shadow only on the hero. Every other surface is flat fill. Heads are native Button views with a ButtonStyle that covers default, pressed, focused, disabled, and selected, .contentShape on the fill, min 44pt. Interactive lists and sheets use Button, not onTapGesture. One colour accessor, one type accessor, one space accessor, one radius accessor. Reduce Motion keeps fade and drops the 0.97 press travel. take marrow is kit reuse only. New types, new layout, new motion.

### 3.3 Naming contract

Convention: Spoonerism / marrowsky lexicon.

Examples to follow: `Onset`, `Head`, `mend(_:)`, `MuffMark`

### 3.4 Dependency contract

None. project.yml has no packages key. Do not add SPM or a bundled font. SF Pro is the system face. The leftover AVCaptureMetadataOutput scanner and cgi search pl endpoint are unused. Do not link AVFoundation for capture, do not request a camera permission, and do not call a remote catalog. Explore reads a bundled Museo Nacional del Prado shelf on device. Empty or failed lookup stays on that shelf.

### 3.5 Navigation contract

Onset-locked chrome. Quiz is the only root and the spoonered Line never leaves. Explore, Saved, and Settings arrive as sheets. Spoon and mend fuse on Quiz. There is no TabView. -ReviewScreen today opens Quiz after onboarding, log opens the Saved sheet, goals opens the Settings sheet, explore opens the Explore sheet. Read ProcessInfo arguments once after onboarding is done. Sheets dismiss. Undo lives on Quiz.

### 3.6 Screen composition contract

Onset-root fused quiz (Quiz holds the painting and the spoonered line of heads; Explore, Saved and Settings are sheets) · marrow. Physical screens: Onboarding (three pages, Continue full width at the bottom), Quiz (onset root: painting tile, Line of Heads, Fluent empty as a full page), Explore sheet (Prado shelf, save Idle, duplicate object id focuses the row), Saved sheet (Mended works plus MendMarks listed with MuffMarks), Settings sheet (contact-us, tappable Prado credit, reset, re-run onboarding). Fluent is not a pushed scene. No three-tab bar.
...
Scheme: **By onset role (Onset, Work, Line, Head, MendMark, MuffMark) · marrow**
...
Direction: **Soft card daylight · take marrow**
...
SF Pro via Font.system. The type move is short display, max four words, tight leading, and a small body under it. One accessor of six Dynamic Type steps: display for the Line field label, title for the painting name under the tile, headline for sheet titles, body at about 17pt for Prado credit and empty copy, caption for MendMark and MuffMark rows, micro for daykey and accession. Display stays one or two lines, never more than four. MendMark counts, MuffMark counts, and YYYYMMDD day keys go through NumberFormatter with tabular figures. @ScaledMetric for any custom size. Headlines must not clip at AX5.
...
This app's `ui` axis is **SwiftUI pure · take marrow**.
...
If **SwiftUI pure · take marrow** is already fully native (no custom drawing layer), this section is satisfied automatically — there is nothing to confine.
...
One Codable Onset document encoded to JSON in UserDefaults under a versioned key such as spn.onset.v1. The snapshot holds Works with Idle Spooned Mended, the live Line and Heads, MendMarks, MuffMarks, onboarding completion, and the Prado object ids. UI never reads UserDefaults. Writes debounce, then flush when scenePhase is inactive or background, and after Mend, Muff, Undo, Explore save, and reset. Decoding failure falls back to empty Fluent and surfaces a calm reset. resetAllData() is reachable from Settings. Simulator seed behind spn.demo.v1 writes several Idle works, one live Line, and onboarding complete. Never seed on a device. Day edges use Calendar.current.startOfDay. Day keys are Int in YYYYMMDD form.
...
Set `User-Agent: Spoonerism/1.0 (iOS; +https://spoonerism-marrow.pro)` on every request. Never reuse another app's string.
...
## 12. Functional twist: Spoon-then-mend (Spoon samples a Work that is not Mended whose chosen field has at least two tokens with unlike first letters; a Line is artist XOR title with the first letters of two Heads swapped; Mend writes a MendMark when the tapped Head is one of the swapped pair, rights both letters, and folds Spooned to Mended; a miss writes a MuffMark and keeps the Line; Mend on Idle is refused; Spoon on a field with fewer than two unlike first letters is refused)

Spoon-then-mend is the home mechanic. Launch already paints a live Line so the first Head hit can file. Spoon draws one saved painting that is not Mended and whose picked field holds two or more words with unlike opening letters, then paints maker or title, never both, as Heads with those two first letters exchanged. Hitting a swapped Head rights both letters, records a MendMark, and moves Spooned to Mended so the work leaves the Spoon pool. Hitting any other Head records a MuffMark, greys that Head, and leaves the Line up. Mend does nothing before Spoon. A later Spoon while Spooned does nothing. Spoon on a field with fewer than two unlike first letters is refused. Saved lists MendMarks together with MuffMarks. Settings names Museo Nacional del Prado.
...
Art style: **3D glass render glassmorphism · take marrow**
...
given. Every name carries the `spn_` prefix.
...
| --- | --- | --- | --- | --- |
| 1 | `spn_AppIcon` | 1024x1024 | **NO** | App Store icon. NO alpha channel, NO transparency, NO text, NO rounded corners, NO drop shadow outside the canvas. |
| 2 | `spn_Splash` | 1290x2796 | fill | Launch background. The middle third must stay quiet so the wordmark reads on top. |
| 3 | `spn_Onboarding1` | 1024x1536 | **required cutout** | Onboarding page 1 illustration: what the app is for. |
| 4 | `spn_Onboarding2` | 1024x1536 | **required cutout** | Onboarding page 2 illustration: the main verb. |
| 5 | `spn_Onboarding3` | 1024x1536 | **required cutout** | Onboarding page 3 illustration: why they stay. |
| 6 | `spn_EmptyHome` | 1024x1024 | **required cutout** | Empty state: the home screen has nothing yet. Calm and inviting, never sad. |
| 7 | `spn_EmptyList` | 1024x1024 | **required cutout** | Empty state: a secondary list has no rows. |
| 8 | `spn_CardBackdrop` | 1200x800 | fill | Backdrop art for a primary card. Low contrast so text stays readable. |
| 9 | `spn_ControlFace` | 512x512 | **required cutout** | Custom control artwork used for the primary interactive element. |
| 10 | `spn_TwistHero` | 1024x1024 | **required cutout** | Hero art for the 'Spoon-then-mend (Spoon samples a Work that is not Mended whose chosen field has at least two tokens with unlike first letters; a Line is artist XOR title with the first letters of two Heads swapped; Mend writes a MendMark when the tapped Head is one of the swapped pair, rights both letters, and folds Spooned to Mended; a miss writes a MuffMark and keeps the Line; Mend on Idle is refused; Spoon on a field with fewer than two unlike first letters is refused)' feature screen. |
| 11 | `spn_SuccessMark` | 512x512 | **required cutout** | Shown briefly when the primary action succeeds. |
| 12 | `spn_HeaderDecor` | 1200x600 | **required cutout** | Decorative header accent on the main screen. |

### Prompt per asset

**`spn_AppIcon`** — 1024x1024
...
Add a unit test target `SpoonerismTests` covering at minimum:
...
- [ ] `xcodebuild -scheme Spoonerism -destination 'generic/platform=iOS' build` succeeds.
...
**Uniqueness**
- [ ] Architecture matches **Spoonerism ADT fold (Idle | Spooned | Mended); the onset is a fold over Works; Spoon writes a Line that is artist XOR title with the first letters of two Heads swapped and folds Idle to Spooned; Mend writes a MendMark when the tapped Head is one of the swapped pair, rights both letters, and folds Spooned to Mended; a miss writes a MuffMark and keeps the Line; Mend on Idle is refused; a second Spoon while Spooned is refused; Spoon samples a Work that is not Mended whose chosen field has at least two tokens with unlike first letters; empty onset writes Fluent** with no leakage across layers.
- [ ] UI approach matches **SwiftUI pure · take marrow**.
- [ ] Custom rendering, if any, is confined to one hero surface (section 7.5).
- [ ] Navigation matches **Onset-locked chrome (the spoonered line never leaves; Explore, Saved and Settings arrive as sheets; spoon and mend fuse on Quiz) · marrow**.
